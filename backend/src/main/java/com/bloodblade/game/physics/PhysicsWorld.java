 ...
 * - Detección de impacto: distancia + arco frontal (no usa la pose 3D del
 * cliente)
 * - Zonas de daño: cabeza / torso / piernas según dirección del swing
 * ({@link com.bloodblade.game.model.SwingDirection})
 */
@Component
public class PhysicsWorld {
    ...
    /**
     * Resuelve solapamientos entre todas las cápsulas de jugadores (empuje
     * lateral).
     */
    ...
    /**
     * Resuelve colisiones entre jugadores y objetos destructibles
     * (Muros/Castillos).
     */
    public void resolveWorldCollisions(Collection<Player> players,
            Collection<com.bloodblade.game.model.Destructible> world) {
    ...
    public HitResult detectHit(Player attacker, Player defender) {
        if (!attacker.alive || !defender.alive)
            return new HitResult(false, null, 0);
        ...
        // Detectar choque de espadas: si el defensor tiene arma activa y sus
        ...
        // Si el defensor bloquea explícitamente se usa blockDir; si está
        // cargado/atacando se usa swingDir.
